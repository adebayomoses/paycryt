...
- ✅ **Real-chain live server:** `PAYCRYT_SANDBOX=false` now runs the server on real Tron/EVM/Bitcoin adapters and real CoinGecko/Binance rates, configured from environment variables and validated at startup. It refuses to run on fake chains or made-up prices, requires a persistent database, skips already-used addresses, fails closed with a 503 when a rate or address check can't be completed, ignores funds that were at an address before a request existed, polls without overlapping, and exposes `GET /v1/status`. Driven end to end against real CoinGecko, Binance, Blockstream and an Ethereum RPC. That run found that two real rate sources can disagree by ~14% on the naira (now explained in the error, and tunable) and that free public RPCs intermittently refuse `eth_getLogs`. See [docs/live-server.md](docs/live-server.md).
- 🚧 Verify Paystack/Flutterwave adapters against live sandboxes; add provider webhooks for payout status
- 🚧 A real payment through the whole flow on a testnet (Tron Shasta/Nile, Sepolia) or with a small mainnet amount
- 🚧 Rate limiting and an admin audit log for the server
