...
- The reference server has no rate limiting, no audit log of admin actions, and no per-merchant wallets (all tenants draw sandbox addresses from one deriver). Merchant API keys are stored hashed and tenants are isolated (see docs/multi-tenant.md), but it is still a starting point, not a hardened service. `node:sqlite` (used by `SqliteStore`) is itself an experimental Node API.
